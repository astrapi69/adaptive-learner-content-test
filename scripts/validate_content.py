...
def _run_index(check: bool) -> int:
    """Delegate to generate_search_index.py (kept as a separate script so
    the search index is generated by exactly one code path)."""
    try:
        import generate_search_index
    except ImportError:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        import generate_search_index
    return generate_search_index.main(["--check"] if check else [])


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate adaptive-learner content")
    parser.add_argument(
        "--generate-index",
        action="store_true",
        help="Regenerate search-index.json via generate_search_index.py.",
    )
    parser.add_argument(
        "--check-index",
        action="store_true",
        help="Fail if search-index.json is stale (does not run content checks).",
    )
